...
**Releasing:** bump `version` in `package.json`, update `CHANGELOG.md`, merge, then publish a GitHub release tagged `v<version>`. The *Publish to npm* workflow checks, tests, builds and publishes it with provenance. It uses npm [trusted publishing](https://docs.npmjs.com/trusted-publishers) (OIDC), so no npm token is stored anywhere.
